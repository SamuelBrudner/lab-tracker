...
The guided Setup page can also stage a grant abstract, aims, or project brief
to seed initial questions. Before uploading that context, the researcher must
explicitly acknowledge that it will be sent to the operator's configured
external AI provider. The browser uses opaque deterministic retry keys, and
the UI calls a draft “ready” only when the server returns the `ready` status;
queued and failed attempts remain visibly distinct and keep the source text
available.

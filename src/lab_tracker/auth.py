        ...
        *,
        ...
        if self._session_factory is None:
            ...
            with invitation_token_service._memory_lock, self._memory_lock:
                ...
                row.consumed_by_user_id = user.user_id
                return user
    ...
    def revoke_invitation(self, invitation_id: UUID) -> Invitation:
        ...
        with self._session_factory() as session:
            ...
            if result.rowcount == 1:
                ...
                session.rollback()
            persistent_row = session.get(InvitationModel, str(invitation_id))
            if persistent_row is None:
                raise NotFoundError("Invitation does not exist.")
            if persistent_row.consumed_at is not None:
                raise ValidationError("Consumed invitations cannot be revoked.")
            return _invitation_from_model(persistent_row)

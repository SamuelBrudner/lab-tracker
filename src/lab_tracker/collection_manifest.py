...
def canonicalize_collection_manifest(
    *,
    ...
    if schema_version != COLLECTION_MANIFEST_SCHEMA_VERSION:
        raise ValidationError(
            "Collection manifest schema_version must be "
            f"{COLLECTION_MANIFEST_SCHEMA_VERSION}."
        )
    ...
    for raw_member in raw_members:
        ...
        if not isinstance(checksum_value, str) or not _SHA256_RE.fullmatch(
            checksum_value
        ):
            raise ValidationError(
                f"Collection member checksum must be lowercase SHA-256: {path}"
            )
        ...
        if (
            ...
            raise ValidationError(
                "Collection member size_bytes must be a nonnegative 64-bit "
                f"integer: {path}"
            )
    ...
    if len(canonical_bytes) > MAX_COLLECTION_MANIFEST_BYTES:
        raise ValidationError(
            "Canonical collection manifest exceeds the 16 MiB request limit."
        )
...
def _canonical_path(value: Any) -> str:
    ...
    if value != value.strip():
        raise ValidationError(
            "Collection member path must not have surrounding whitespace."
        )
    if len(value) > MAX_COLLECTION_MEMBER_PATH_LENGTH:
        raise ValidationError(
            "Collection member path must be 1000 characters or fewer."
        )
    if "\x00" in value or "\\" in value:
        raise ValidationError(
            "Collection member path must be a NUL-free relative POSIX path."
        )
    if value.startswith("/"):

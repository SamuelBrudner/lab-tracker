...
function ReviewPanel({
  ...
  async function handleDownloadDatasetFile(file) {
    if (!token || !dataset || !file?.file_id) {
      return;
    }
    try {
      await downloadProtectedResource({
        path: `/datasets/${dataset.dataset_id}/files/${file.file_id}/download`,
        token,
        filename: file.path,
      });
    } catch (err) {
      if (typeof onFlash === "function") {
        onFlash("", err.message || "Failed to download dataset file.");
      }
    }
  }
                        ...
                        <div className="item" key={file.file_id || file.path}>
                          ...
                          {file.file_id ? (
                            <button
                              type="button"
                              className="link"
                              onClick={() => handleDownloadDatasetFile(file)}
                            >
                              Download
                            </button>
                          ) : null}

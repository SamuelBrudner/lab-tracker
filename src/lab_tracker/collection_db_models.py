...
class AcquisitionCollectionModel(Base):
    ...
    # Typed identifiers avoid circular DDL dependencies. Services only point
    # them at persisted rows belonging to this collection.
...
class DatasetCollectionSnapshotLinkModel(Base):
    """Relational integrity edge for a compact Dataset manifest reference."""

    __tablename__ = "dataset_collection_snapshots"
    ...
    snapshot_id: Mapped[UUID] = mapped_column(
        GUID,
        ForeignKey("acquisition_collection_snapshots.snapshot_id"),
        primary_key=True,
    )

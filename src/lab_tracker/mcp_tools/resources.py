...
def lab_tracker_quickstart() -> str:
    return (
        ...
        "When API authentication is enabled, prefer an `lpat_` bearer token in "
        "`LAB_TRACKER_TOKEN`; username/password is a local service-account "
        "fallback. Notes use `staged`, `committed`, or `archived` status. "
        "Note metadata values may be strings, "

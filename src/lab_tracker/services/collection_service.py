...
class AcquisitionCollectionService(BaseService):
        ...
        with self.application_transaction(), self.unit_of_work() as repository:
            ...
            if snapshot is None:
                snapshot = AcquisitionCollectionSnapshot(
                    ...
                    capture_principal_instance_id=_capture_principal_instance_id(actor),
                    capture_principal_label=_capture_principal_label(actor),
                    created_at=now,
                    updated_at=now,
                )
            ...
            capture = AcquisitionCollectionCapture(
                ...
                source_uri=resolved_source_uri,
                capture_actor_user_id=capture_actor_user_id,
                capture_principal_type=(actor.principal_type.value if actor is not None else None),
                capture_principal_instance_id=_capture_principal_instance_id(actor),
                capture_principal_label=_capture_principal_label(actor),
                created_at=now,
            )
    ...
    def get_collection_for_read(
        ...
        *,
        ...
        try:
            ...
            raise OpaqueTargetNotFoundError("Acquisition collection does not exist.") from exc
        if not self.authorization.can_read(session.project_id, actor=actor):
            raise OpaqueTargetNotFoundError("Acquisition collection does not exist.")
        return collection
    ...
    def list_collections(
        ...
        *,
        ...
        self.sessions.get_session_for_read(session_id, actor=actor)
    ...
    def get_snapshot_for_read(
        ...
        *,
        ...
        try:
            ...
            collection = self.get_collection(snapshot.collection_id)
            session = self.sessions.get_session(collection.session_id)
        except NotFoundError as exc:
            raise OpaqueTargetNotFoundError(
                "Acquisition collection snapshot does not exist."
            ) from exc
        if not self.authorization.can_read(session.project_id, actor=actor):
            raise OpaqueTargetNotFoundError("Acquisition collection snapshot does not exist.")
        return snapshot
    ...
    def list_snapshots(
        ...
        *,
        ...
        collection = self.get_collection_for_read(collection_id, actor=actor)
        snapshots, total = self.repository.acquisition_collections.query_snapshots(
            collection_id=collection.collection_id,
            limit=limit,
            offset=offset,
        )
    ...
    def get_manifest(
        ...
        *,
        ...
        self.get_snapshot_for_read(snapshot_id, actor=actor)
    ...
    def list_members(
        ...
        *,
        ...
        if needle:
            members = [member for member in members if needle in member.path.casefold()]
        total = len(members)
    ...
    *,
    ...
    if current_observed_at is not None and observed_at == current_observed_at:
        if collection.current_snapshot_id == snapshot.snapshot_id:
            if collection.current_capture_id == capture.capture_id:
                return False
        ...
        if (
            ...
            return False
        raise ConflictError("A different collection snapshot already has the same observed_at.")
    collection.current_snapshot_id = snapshot.snapshot_id
    collection.current_capture_id = capture.capture_id
...
def _capture_principal_instance_id(
    actor: AuthContext | None,
) -> UUID | None:
    """Resolve provenance without requiring the later principal-profile slice."""

    if actor is None:
        return None
    explicit = getattr(actor, "principal_instance_id", None)
    if isinstance(explicit, UUID):
        return explicit
    if actor.principal_type == PrincipalType.USER:
        return actor.user_id
    if actor.principal_type == PrincipalType.DEVICE:
        return actor.device_token_id
    return None


def _capture_principal_label(actor: AuthContext | None) -> str | None:
    if actor is None:
        return None
    label = getattr(actor, "principal_label", None)

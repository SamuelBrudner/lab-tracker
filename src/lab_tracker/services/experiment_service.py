...
class ExperimentService(BaseService):
    ...
    def create_experiment(
        ...
        *,
        ...
        with self.application_transaction(), self.unit_of_work() as repository:
            # Question deletion takes this same project-scoped lock. Re-read
            # afterward so the Experiment FK and the friendly delete guard
            # agree even when create and delete race.
            repository.lock_project_question_dag(project_id)
            ...
            if question.status != QuestionStatus.ACTIVE:
                raise ValidationError("Primary question must be active for Experiments.")
    ...
    def get_experiment(self, experiment_id: UUID) -> Experiment:
        return cast(
            ...
            self.get_from_repository(
                ...
                label="Experiment",
                loader=lambda repository: repository.experiments.get(experiment_id),
            ),
        )

    def get_experiment_for_read(
        self,
        experiment_id: UUID,
        *,
        actor: AuthContext | None = None,
        ...
        try:
            ...
            raise OpaqueTargetNotFoundError("Experiment does not exist.") from exc
        if not self.authorization.can_read(experiment.project_id, actor=actor):
            raise OpaqueTargetNotFoundError("Experiment does not exist.")
        return experiment
    ...
    def update_experiment(
        ...
        *,
        ...
        if name is None and isinstance(description, _UnsetDescription) and status is None:
            return experiment
        with self.application_transaction(), self.unit_of_work() as repository:
            ...
            now = utc_now()
            if status == ExperimentStatus.CLOSED and experiment.status == ExperimentStatus.ACTIVE:
                experiment.closed_at = now
            if status == ExperimentStatus.ARCHIVED and experiment.status == ExperimentStatus.CLOSED:
    ...
    def add_session(
        ...
        *,
        ...
        with self.application_transaction(), self.unit_of_work() as repository:
            # Session lifecycle commands take this same lock, so membership
            # changes are wholly before or after promotion or deletion.
            ...
            if experiment.status != ExperimentStatus.ACTIVE:
                raise ValidationError("Only active Experiments accept Session members.")
    ...
    def remove_session(
        ...
        *,
        ...
        with self.application_transaction(), self.unit_of_work() as repository:
            # Keep DELETE ordered with Session promotion and deletion.
            repository.lock_session_acquisition_state(session_id)
            repository.lock_experiment_updates((experiment_id,))
            experiment = self.get_experiment(experiment_id)
            self.authorization.require_contributor(
    ...
    def add_dataset(
        ...
        *,
        ...
        with self.application_transaction():
            ...
            self.repository.lock_dataset_updates(
                experiment.project_id,
                (dataset_id,),
            )
    ...
    def _add_dataset_locked(
        ...
        *,
        ...
        if self.repository.experiment_has_dataset(
            ...
            return experiment
        if experiment.status == ExperimentStatus.ARCHIVED:
            raise ValidationError("Archived Experiments do not accept Dataset members.")
    ...
    def _ensure_status_transition(
        ...
        if next_status not in EXPERIMENT_STATUS_TRANSITIONS[current]:
            raise ValidationError(
                f"Experiment status cannot transition from {current.value} to {next_status.value}."
            )
    ...
    def _validate_session(
        ...
        if session.project_id != experiment.project_id:
            raise ValidationError("Session must belong to the same project as the Experiment.")
        if (
            session.session_type == SessionType.SCIENTIFIC
            and session.primary_question_id != experiment.primary_question_id
        ):
            raise ValidationError(
                "Scientific Session primary question must match the Experiment primary question."
            )
    ...
    def _validate_dataset(
        ...
        if dataset.project_id != experiment.project_id:
            raise ValidationError("Dataset must belong to the same project as the Experiment.")
        if not any(
            link.question_id == experiment.primary_question_id for link in dataset.question_links
        ):
            raise ValidationError("Dataset must link the Experiment primary question.")

...
class SQLAlchemyAcquisitionCollectionRepository:
    ...
    def get_or_create(
        ...
        if persisted is None:
            raise RuntimeError("Collection upsert did not return a persisted row.")
        return persisted
    ...
    def query(
        ...
        *,
        ...
        if session_id is not None:
            stmt = stmt.where(AcquisitionCollectionModel.session_id == str(session_id))
            count_stmt = count_stmt.where(AcquisitionCollectionModel.session_id == str(session_id))
        stmt = stmt.order_by(
        ...
        if snapshot_ids:
            snapshot_rows = self._session.scalars(
                select(AcquisitionCollectionSnapshotModel).where(
                    AcquisitionCollectionSnapshotModel.snapshot_id.in_(snapshot_ids)
                )
            )
            snapshots = {row.snapshot_id: _snapshot_from_row(row) for row in snapshot_rows}
        capture_ids = [row.current_capture_id for row in rows if row.current_capture_id is not None]
        ...
        if capture_ids:
            capture_rows = self._session.scalars(
                select(AcquisitionCollectionCaptureModel).where(
                    AcquisitionCollectionCaptureModel.capture_id.in_(capture_ids)
                )
            )
            captures = {row.capture_id: _capture_from_row(row) for row in capture_rows}
        return (
            [
                AcquisitionCollectionSummary(
    ...
    def get_snapshot_by_hash(
        ...
        *,
        ...
        row = self._session.scalar(
            select(AcquisitionCollectionSnapshotModel).where(
                AcquisitionCollectionSnapshotModel.collection_id == str(collection_id),
                AcquisitionCollectionSnapshotModel.manifest_hash == manifest_hash,
            )
        )
        return None if row is None else _snapshot_from_row(row)
    ...
    def query_snapshots(
        ...
        *,
        ...
        count_stmt = select(AcquisitionCollectionSnapshotModel.snapshot_id).where(
            AcquisitionCollectionSnapshotModel.collection_id == str(collection_id)
        )
        latest_capture_observed_at = (
            select(func.max(AcquisitionCollectionCaptureModel.observed_at))
            .where(
                AcquisitionCollectionCaptureModel.snapshot_id
                == AcquisitionCollectionSnapshotModel.snapshot_id
            )
            .correlate(AcquisitionCollectionSnapshotModel)
            .scalar_subquery()
        )
        stmt = stmt.order_by(
            func.coalesce(
                latest_capture_observed_at,
                AcquisitionCollectionSnapshotModel.observed_at,
            ).desc(),
        ...
        if row is None:
            self._session.add(
                AcquisitionCollectionSnapshotModel(
                    ...
                    capture_principal_instance_id=(snapshot.capture_principal_instance_id),
    ...
    def get_capture(
        ...
        *,
        ...
        row = self._session.scalar(
            select(AcquisitionCollectionCaptureModel).where(
                AcquisitionCollectionCaptureModel.collection_id == str(collection_id),
                AcquisitionCollectionCaptureModel.client_capture_id == client_capture_id,
            )
        )
    ...
    def get_latest_capture_for_snapshot(
        ...
        row = self._session.scalar(
            ...
            .where(AcquisitionCollectionCaptureModel.snapshot_id == str(snapshot_id))
            .order_by(
                AcquisitionCollectionCaptureModel.observed_at.desc(),
                AcquisitionCollectionCaptureModel.created_at.desc(),
                AcquisitionCollectionCaptureModel.capture_id.desc(),
            )
            .limit(1)
        )
        return None if row is None else _capture_from_row(row)

...
def _add_person_with_supervision(
    ...
    *,
    ...
    relationships = _supervision_relationships(person_node)
    relationship_keys = {_supervision_relationship_key(item) for item in relationships}
    ...
    for edge in sorted(active_edges, key=lambda value: (value.started_at, str(value.edge_id))):
        ...
        key = _supervision_relationship_key(relationship)
        if key in relationship_keys:
            continue
        relationships.append(relationship)
        relationship_keys.add(key)

    _set_supervision_relationships(person_node, relationships)


def _supervision_relationships(person_node: dict[str, Any]) -> list[dict[str, object]]:
    existing_relationships = person_node.get("actedOnBehalfOf")
    if isinstance(existing_relationships, list):
        return list(existing_relationships)
    if isinstance(existing_relationships, dict):
        return [existing_relationships]
    return []


def _supervision_relationship_key(relationship: dict[str, object]) -> tuple[str, str, str]:
    return (
...
def _set_supervision_relationships(
...
def _merged_person_node(
    primary: dict[str, Any],
    secondary: dict[str, Any],
) -> dict[str, Any]:
    """Return ``primary`` extended with ``secondary``'s supervision relationships.

    One person can appear in several sub-documents (dataset commit, analysis
    execution, claim creation), each evaluating supervision at its own activity
    time; the merged node keeps every relationship instead of whichever copy was
    written last.
    """

    person_node = dict(primary)
    relationships = _supervision_relationships(person_node)
    relationship_keys = {_supervision_relationship_key(item) for item in relationships}
    for relationship in _supervision_relationships(secondary):
        key = _supervision_relationship_key(relationship)
...
def _merge_person_nodes(
    ...
    for user_id in sorted(people):
        ...
        merged[node_id] = (
            dict(person) if existing is None else _merged_person_node(person, existing)
        )
...
def build_claim_provenance_document(
    ...
    *,
    ...
    provided_analysis_ids = {analysis.analysis_id for analysis in analyses}
    for visualization in visualizations:
        if visualization.analysis_id not in provided_analysis_ids:
            raise ValueError(
                f"Visualization {visualization.viz_id} is generated by analysis "
                f"{visualization.analysis_id}, which was not provided to the claim "
                "provenance builder."
            )
...
def _merge_graph_nodes(
    ...
    for node in graph:
        ...
        elif _node_type_includes(existing, "prov:Person") and _node_type_includes(
            node, "prov:Person"
        ):
            merged[node_id] = _merged_person_node(existing, node)

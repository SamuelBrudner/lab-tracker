...
For a new database, generate a URL-safe value such as `openssl rand -hex 32`;
the password is interpolated into a SQLAlchemy URL. Preserve the existing value
for an existing deployment.

The provider credential remains in the ignored
`deployments/shared-provider/.env`. Compose uses that file only for variable
substitution and passes the single named provider credential to the app; it
does not inject the whole file. Neither file belongs in an image or commit.
...
commands succeed. The backup pauses the app while it captures both artifacts so
they describe one maintenance-window checkpoint, then resumes it before archive
validation. Backups contain plaintext research and authentication data; keep
the generated restrictive permissions, apply the operator's encrypted storage
and retention policy, and never upload them to source control.
...
restores the previous immutable app image ID automatically; the validated
backup remains the recovery point for any database-level rollback. The Compose file requires

...
def test_marion_compose_is_isolated_and_fails_closed_on_image() -> None:
    ...
    assert "LAB_TRACKER_OPENAI_API_KEY: ${LAB_TRACKER_OPENAI_API_KEY:-}" in compose
    assert "\n    env_file:" not in compose
    assert "LAB_TRACKER_OPENAI_MODEL: gpt-5.6-sol" in compose
    assert "LAB_TRACKER_OPENAI_REASONING_EFFORT: xhigh" in compose
    assert "LAB_TRACKER_OPENAI_REASONING_MODE: standard" in compose
    assert 'LAB_TRACKER_OPENAI_TIMEOUT_SECONDS: "5400"' in compose
    assert (
...
def test_backup_script_targets_only_marion_and_validates_archives() -> None:
    ...
    assert "pause app" in script
    assert "unpause app" in script
...
def test_release_script_requires_a_clean_revision_and_verifies_live_health() -> None:
    ...
    assert 'MARION_IMAGE="${previous_image_id}"' in script

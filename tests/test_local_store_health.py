...
def test_ordinary_probe_exception_is_logged_without_its_message(
    ...
    secret = "private local path diagnostic"
    inspector = RecordingInspector((RuntimeError(secret),))
    target = _target("/raw/operator/secret-store")

    with caplog.at_level(logging.WARNING, logger="lab_tracker.local_store_health"):
        result = _probe(inspector)(target)

    _assert_static_failure(result)

    records = [
        record for record in caplog.records if record.name == "lab_tracker.local_store_health"
    ]
